...
# ---- Optional PDF libraries (guarded) ----
try:
    from pypdf import PdfReader as PYPDF_Reader, PdfWriter as PYPDF_Writer
except ImportError:
    PYPDF_Reader = None
    PYPDF_Writer = None

try:
    from PyPDF2 import PdfReader as PYPDF2_Reader, PdfWriter as PYPDF2_Writer
except ImportError:
    PYPDF2_Reader = None
    PYPDF2_Writer = None
...
# ---- Optional PDF viewer (pdf.js-based) ----
try:
...
PDF_SUMMARY_SYSTEM_PROMPT_ZH = """
你是一位專業的長文件分析與摘要顧問，擅長閱讀合併後的 PDF 文件。
請嚴格使用繁體中文回答，並使用 Markdown 格式輸出。
"""

PDF_QA_SYSTEM_PROMPT_ZH = """
你是一位專業文件顧問，會根據合併後 PDF 的全文內容回答使用者問題。
請：
- 嚴格根據提供的文字內容作答，避免臆測。
- 回答語言使用繁體中文。
- 以 Markdown 格式排版，必要時可用標題、條列與表格。
"""

OCR_LLM_SYSTEM_PROMPT = """
You are an OCR transcription assistant.
Your job is to read page images of PDF documents and output ONLY the recognized text, in reading order.
- Do NOT summarize, translate, or explain.
- Preserve English text as-is.
- For Chinese text, use Traditional Chinese characters (do NOT convert to Simplified).
...
def get_api_key(name: str, ui_value: Optional[str]) -> Optional[str]:
    """Prefer environment variable; fallback to UI input. Never display env value."""
...
def available_merge_backends() -> Dict[str, bool]:
    return {
        "pypdf": PYPDF_Reader is not None and PYPDF_Writer is not None,
        "PyPDF2": PYPDF2_Reader is not None and PYPDF2_Writer is not None,
        "PyMuPDF": fitz is not None,
    }


def pdf_lib_available() -> bool:
    backends = available_merge_backends()
    return any(backends.values())


def render_pdf_viewer(pdf_bytes: bytes, height: int = 600, key: str = "pdf_viewer"):
    """
    Prefer streamlit-pdf-viewer (pdf.js-based, better for Edge); fallback to iframe.
    """
    if st_pdf_viewer is not None:
        st_pdf_viewer(pdf_bytes, height=height, key=key)
    else:
...
def merge_pdf_bytes(pdf1: bytes, pdf2: bytes, backend: str) -> bytes:
    """
    Merge two PDFs using selected backend: 'pypdf', 'PyPDF2', or 'PyMuPDF'.
    """
    if backend == "pypdf":
        if not (PYPDF_Reader and PYPDF_Writer):
            raise RuntimeError("pypdf is not installed.")
        writer = PYPDF_Writer()
        for raw in (pdf1, pdf2):
            reader = PYPDF_Reader(BytesIO(raw))
    ...
    if backend == "PyPDF2":
        if not (PYPDF2_Reader and PYPDF2_Writer):
            raise RuntimeError("PyPDF2 is not installed.")
        writer = PYPDF2_Writer()
        for raw in (pdf1, pdf2):
            reader = PYPDF2_Reader(BytesIO(raw))
            for page in reader.pages:
                writer.add_page(page)
        out = BytesIO()
        writer.write(out)
    ...
    raise RuntimeError(f"Unsupported PDF merge backend: {backend}")


def extract_text_from_pdf_bytes(pdf_bytes: bytes) -> str:
    """
    Extract text from PDF using first available backend:
    - pypdf
    - PyPDF2
    - PyMuPDF
    """
    if PYPDF_Reader is not None:
        reader = PYPDF_Reader(BytesIO(pdf_bytes))
        pages_text: List[str] = []
    ...
    if PYPDF2_Reader is not None:
        reader = PYPDF2_Reader(BytesIO(pdf_bytes))
        pages_text: List[str] = []
        for idx, page in enumerate(reader.pages):
            try:
                txt = page.extract_text() or ""
    ...
    raise RuntimeError("No PDF library available for text extraction.")
...
# ---------- OCR HELPERS (Tesseract & LLM-based) ----

def build_text_with_tesseract_ocr(
    pdf_bytes: bytes,
    pages_to_ocr: List[int],
    lang_code: str,
) -> str:
    """
    Build a full-text view of the PDF, using Tesseract OCR for selected pages
    and PyMuPDF text extraction for others.
    """
    if fitz is None or pytesseract is None or Image is None:
        raise RuntimeError("Tesseract OCR requires PyMuPDF, pytesseract, and Pillow.")
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    ...
    for idx, page in enumerate(doc):
        ...
        if pno in page_set:
            ...
            text = pytesseract.image_to_string(image, lang=lang_code) or ""
            parts.append(f"[Page {pno} - OCR]\n{text.strip()}")
        else:
...
def openai_ocr_single_page(
    ...
    if lang_mode == "English":
        lang_hint = "Focus on English text. "
    elif lang_mode == "Traditional Chinese":
        lang_hint = "Focus on Traditional Chinese (繁體中文) text. "
    elif lang_mode == "English + Traditional Chinese":
        lang_hint = "Recognize both English and Traditional Chinese (繁體中文) text. "

    system_prompt = OCR_LLM_SYSTEM_PROMPT + "\n" + lang_hint
    user_instruction = "Please read the attached page image and output only the recognized text."
...
def gemini_ocr_single_page(
    ...
    resp = model_obj.generate_content(
        [
            "Please read the attached page image and output only the recognized text.",
...
def build_text_with_llm_ocr(
    ...
    """
    Build full-text view: LLM OCR on selected pages, PyMuPDF text on others.
    """
    if fitz is None:
        raise RuntimeError("LLM-based OCR requires PyMuPDF (fitz).")

    provider = "openai" if model_name.startswith("gpt-") else "gemini"
    if provider == "openai" and not api_keys.get("openai"):
        raise RuntimeError("OpenAI API key missing for LLM-based OCR.")
    if provider == "gemini" and not api_keys.get("gemini"):
        raise RuntimeError("Gemini API key missing for LLM-based OCR.")
    ...
    for idx, page in enumerate(doc):
        ...
        if pno in page_set:
            ...
            if provider == "openai":
                ...
                text = gemini_ocr_single_page(
                    api_key=api_keys["gemini"],
                    model=model_name,
                    image_bytes=img_bytes,
                    lang_mode=lang_mode,
                )
            parts.append(f"[Page {pno} - OCR-LLM]\n{text.strip()}")
        else:
...
UI_LABELS = {
    "en": {
        ...
        # PDF tab
        "pdf_upload_title": "PDF Merge, Preview & Analysis",
        "pdf_upload1": "Upload PDF A",
        "pdf_upload2": "Upload PDF B",
        "pdf_zoom": "Viewer zoom (relative height)",
        "pdf_viewer_a": "Preview – PDF A",
        "pdf_viewer_b": "Preview – PDF B",
        "pdf_merge_button": "Merge PDFs",
        "pdf_backend_select": "PDF merge backend (Python package)",
        "pdf_download_merged": "Download merged PDF",
        "pdf_no_lib": "No PDF library found. Please install at least one of: pypdf, PyPDF2, PyMuPDF.",
        "pdf_merged_info": "Merged PDF is ready. Text length used for summary/Q&A (chars)",
        "pdf_summary_section": "1) Auto-summary of merged PDF",
        "pdf_summary_prompt": "Summary task prompt (can edit)",
        "pdf_summary_model": "Summary model",
        "pdf_summary_max_tokens": "Summary max tokens",
        "pdf_summary_temperature": "Summary temperature",
        "pdf_summary_run": "Generate summary",
        "pdf_summary_view_mode": "Summary view",
        "pdf_view_rendered": "Rendered Markdown",
        "pdf_view_raw": "Raw Markdown",
        "pdf_qa_section": "2) Ask questions about merged PDF",
        "pdf_qa_question": "Your question about the merged PDF",
        "pdf_qa_prompt": "Extra instructions for answering (optional)",
        "pdf_qa_model": "Q&A model",
        "pdf_qa_max_tokens": "Q&A max tokens (default 24000)",
        "pdf_qa_temperature": "Q&A temperature",
        "pdf_qa_run": "Run Q&A",
        ...
        "ocr_backend_tesseract": "Tesseract (local OCR)",
        "ocr_backend_llm": "LLM-based OCR",
        "ocr_lang": "OCR language",
        "ocr_lang_en": "English",
        "ocr_lang_zh": "Traditional Chinese",
        "ocr_lang_both": "English + Traditional Chinese",
        "ocr_pages": "Pages to OCR",
        "ocr_run": "Run OCR on selected pages",
        "ocr_llm_model": "OCR LLM model",
        "ocr_result_title": "OCR result (editable text used for summary & Q&A)",
        "ocr_view_mode": "OCR view",
        "ocr_view_edit": "Text editor",
        "ocr_view_preview": "Markdown preview",
        "ocr_info_using_ocr": "Currently using OCR text for summary & Q&A.",
        "ocr_info_using_native": "Currently using original extracted text for summary & Q&A.",
    ...
    "zh": {
        ...
        # PDF tab
        "pdf_upload_title": "PDF 合併、預覽與智能分析",
        "pdf_upload1": "上傳 PDF A",
        "pdf_upload2": "上傳 PDF B",
        "pdf_zoom": "檢視縮放（高度倍率）",
        "pdf_viewer_a": "預覽 – PDF A",
        "pdf_viewer_b": "預覽 – PDF B",
        "pdf_merge_button": "合併兩份 PDF",
        "pdf_backend_select": "PDF 合併使用套件（Python package）",
        "pdf_download_merged": "下載合併後 PDF",
        "pdf_no_lib": "目前找不到任何 PDF 函式庫，請在環境中安裝 pypdf、PyPDF2 或 PyMuPDF。",
        "pdf_merged_info": "目前用於摘要 / 問答的文字長度（字元數）",
        "pdf_summary_section": "一）合併後 PDF 自動摘要",
        "pdf_summary_prompt": "摘要任務提示（可自行調整）",
        "pdf_summary_model": "摘要使用模型",
        "pdf_summary_max_tokens": "摘要最大輸出 tokens",
        "pdf_summary_temperature": "摘要溫度 (temperature)",
        "pdf_summary_run": "產生摘要與實體表格",
        "pdf_summary_view_mode": "摘要檢視模式",
        "pdf_view_rendered": "Markdown 呈現",
        "pdf_view_raw": "原始 Markdown 文字",
        "pdf_qa_section": "二）針對合併後 PDF 提問",
        "pdf_qa_question": "請輸入你對合併後 PDF 的問題",
        "pdf_qa_prompt": "回答風格 / 附加說明（選填）",
        "pdf_qa_model": "問答使用模型",
        "pdf_qa_max_tokens": "問答最大輸出 tokens（預設 24000）",
        "pdf_qa_temperature": "問答溫度 (temperature)",
        "pdf_qa_run": "執行問答",
        ...
        "ocr_backend": "OCR 後端",
        "ocr_backend_tesseract": "Tesseract（本地 OCR）",
        "ocr_backend_llm": "LLM OCR（透過模型進行影像辨識）",
        "ocr_lang": "OCR 語言",
        "ocr_lang_en": "英文",
        "ocr_lang_zh": "繁體中文",
        "ocr_lang_both": "英文 + 繁體中文",
        "ocr_pages": "選擇要做 OCR 的頁碼",
        "ocr_run": "對勾選頁面執行 OCR",
        "ocr_llm_model": "OCR 使用之 LLM 模型",
        "ocr_result_title": "OCR 結果（可編輯，供摘要 / 問答使用）",
        "ocr_view_mode": "OCR 檢視模式",
        "ocr_view_edit": "文字編輯模式",
        "ocr_view_preview": "Markdown 預覽模式",
        "ocr_info_using_ocr": "目前摘要 / 問答將使用 OCR 後的文字內容。",
        "ocr_info_using_native": "目前摘要 / 問答使用的是原始文字擷取結果（未套用 OCR）。",
...
def init_session_state():
    ...
    if "merged_pdf_text_native" not in st.session_state:
        st.session_state.merged_pdf_text_native = ""
    if "merged_pdf_text_ocr" not in st.session_state:
        st.session_state.merged_pdf_text_ocr = ""
    if "merged_pdf_page_count" not in st.session_state:
        st.session_state.merged_pdf_page_count = 0
    if "use_ocr_text" not in st.session_state:
        st.session_state.use_ocr_text = False
    if "pdf_summary_result" not in st.session_state:
        st.session_state.pdf_summary_result = ""
    if "pdf_qa_result" not in st.session_state:
        st.session_state.pdf_qa_result = ""
    if "pdf_summary_prompt" not in st.session_state:
        st.session_state.pdf_summary_prompt = """
請根據下方「合併後 PDF 的全文內容」，產出一份「完整且條理清楚」的摘要報告，要求如下：
...
3. 關鍵字標示（珊瑚色）：
   - 對於特別重要的名詞、關鍵風險、關鍵要求，請在文字外層加上：
     `<span style="color: coral">關鍵字</span>`
   - 請在全文至少標註 20 個以上的關鍵字。

4. 實體抽取與情境表格（20 筆）：
   - 在摘要後方，新增一個 Markdown 表格，表格欄位建議如下：
     | 實體名稱 | 類型 | 相關情境說明 | 可能風險/影響 | 來源頁碼或段落 |
   - 至少列出 **20 個實體（Entity）**，例如：公司名稱、部門、系統、程序、關鍵文件、重要設備、重大風險點等。
   - 「相關情境說明」請用 1–3 句話說明該實體在文件中的角色或用途。
   - 「來源頁碼或段落」若原始文字中無法明確判斷，可填寫「未知 / 需人工補充」。

5. 文字風格：
   - 採用正式、清楚、可以直接提供給管理層閱讀的口吻。
   - 避免過度籠統的描述，盡量具體指出「做了什麼」、「為什麼重要」、「可能的風險或影響」。

若原始內容資訊不足，請明確標註「資訊不足，需人工補充」，不要自行捏造。
"""
    if "pdf_qa_prompt" not in st.session_state:
        st.session_state.pdf_qa_prompt = """
回答原則（可自行調整）：
- 僅根據提供的合併後 PDF 文字內容回答，避免臆測。
- 若文件中沒有相關資訊，請明確說明「在目前提供的內容中，找不到相關資訊」。
- 回答時請使用繁體中文。
...
def provider_for_pdf_model(model_name: str) -> ProviderLiteral:
...
def main():
    ...
    # ----------------- PDF MERGE & ANALYSIS TAB ------

    with tabs[2]:
        st.markdown(f"### {labels['pdf_upload_title']}")
        ...
        if not pdf_lib_available():
            ...
            with col_up1:
                pdf_a = st.file_uploader(
                    labels["pdf_upload1"], type=["pdf"], key="pdf_a"
                )
            with col_up2:
                pdf_b = st.file_uploader(
                    labels["pdf_upload2"], type=["pdf"], key="pdf_b"
                )

            # Merge backend selection
            backend_status = available_merge_backends()
            backend_options = ["pypdf", "PyPDF2", "PyMuPDF"]
            default_backend = next(
                (b for b in backend_options if backend_status.get(b)), "pypdf"
            )
            backend_idx = backend_options.index(default_backend)
            selected_backend = st.selectbox(
                labels["pdf_backend_select"],
                options=backend_options,
                index=backend_idx,
                key="pdf_merge_backend",
            )

            for b in backend_options:
                if backend_status.get(b):
                    st.caption(f"✅ {b} available")
                else:
                    st.caption(f"⚠️ {b} not installed in this environment")
            ...
            viewer_height = int(600 * zoom)

            prev1, prev2 = st.columns(2)
            if pdf_a:
                with prev1:
                    st.caption(labels["pdf_viewer_a"])
                    render_pdf_viewer(pdf_a.getvalue(), height=viewer_height, key="viewer_a")
            if pdf_b:
                with prev2:
                    st.caption(labels["pdf_viewer_b"])
                    render_pdf_viewer(pdf_b.getvalue(), height=viewer_height, key="viewer_b")

            merge_col1, merge_col2 = st.columns([1, 3])
            with merge_col1:
                can_merge = pdf_a is not None and pdf_b is not None
                if st.button(labels["pdf_merge_button"], disabled=not can_merge):
                    try:
                        merged = merge_pdf_bytes(
                            pdf_a.getvalue(), pdf_b.getvalue(), selected_backend
                        )
                        st.session_state.merged_pdf_bytes = merged
                        merged_text = extract_text_from_pdf_bytes(merged)
                        st.session_state.merged_pdf_text_native = merged_text
                        st.session_state.merged_pdf_text = merged_text
                        st.session_state.merged_pdf_text_ocr = ""
                        st.session_state.use_ocr_text = False
                        st.session_state.merged_pdf_page_count = get_pdf_page_count(merged)
                        st.session_state.pdf_summary_result = ""
                        st.session_state.pdf_qa_result = ""
                        log_event(
                            "success",
                            f"Merged two PDFs with backend '{selected_backend}' and extracted text.",
                        )
                    except Exception as e:
                        st.error(f"PDF merge failed: {e}")
                        log_event("error", f"PDF merge failed: {e}")

            merged_bytes = st.session_state.merged_pdf_bytes
            merged_text_effective = st.session_state.merged_pdf_text

            with merge_col2:
                if merged_bytes:
                    st.success(
                        f"{labels['pdf_merged_info']}: {len(merged_text_effective or '')}"
                    )
                    st.download_button(
                        labels["pdf_download_merged"],
                        data=merged_bytes,
            ...
            # --------- OCR (OPTIONAL) ----------
            st.markdown(f"#### {labels['ocr_section']}")
            if not merged_bytes:
                st.info(labels["pdf_need_merged"])
            else:
                ...
                if enable_ocr:
                    ...
                    page_count = st.session_state.merged_pdf_page_count or get_pdf_page_count(
                        merged_bytes
                    )
                    ...
                    if not pages_selected:
                        ...
                        if ocr_backend == labels["ocr_backend_tesseract"]:
                            if pytesseract is None or Image is None or fitz is None:
                                ...
                                if st.button(labels["ocr_run"], key="run_ocr_tesseract"):
                                    try:
                                        with st.spinner("Running Tesseract OCR on selected pages..."):
                                            ocr_text = build_text_with_tesseract_ocr(
                                                merged_bytes, pages_selected, lang_code
                                            )
                                        st.session_state.merged_pdf_text_ocr = ocr_text
                                        st.session_state.merged_pdf_text = ocr_text
                                        st.session_state.use_ocr_text = True
                                        st.session_state.pdf_summary_result = ""
                                        st.session_state.pdf_qa_result = ""
                                        log_event(
                                            "success",
                                            f"Tesseract OCR completed on pages {pages_selected}.",
                                        )
                                    except Exception as e:
                                        st.error(f"OCR failed: {e}")
                                        log_event("error", f"OCR failed: {e}")
                        else:
                            # LLM-based OCR
                            ocr_llm_model = st.selectbox(
                                labels["ocr_llm_model"],
                                options=PDF_MODEL_OPTIONS,
                                key="ocr_llm_model_choice",
                            )
                            lang_choice = st.selectbox(
                                labels["ocr_lang"],
                                options=[
                                ...
                                key="ocr_llm_lang_choice",
                            )
                            lang_mode = lang_choice.replace("英文", "English").replace(
                                "繁體中文", "Traditional Chinese"
                            )

                            if st.button(labels["ocr_run"], key="run_ocr_llm"):
                                try:
                                    ...
                                    log_event(
                                        "success",
                                        f"LLM-based OCR completed on pages {pages_selected}.",
                                    )
                                except Exception as e:
                                    st.error(f"LLM OCR failed: {e}")
                                    log_event("error", f"LLM OCR failed: {e}")

                # OCR result editor / selector
                st.markdown(f"##### {labels['ocr_result_title']}")
                if st.session_state.use_ocr_text:
                    st.info(labels["ocr_info_using_ocr"])
                else:
                    st.info(labels["ocr_info_using_native"])

                ocr_view_mode = st.radio(
                    labels["ocr_view_mode"],
                    options=[labels["ocr_view_edit"], labels["ocr_view_preview"]],
                    key="ocr_view_mode",
                )

                if ocr_view_mode == labels["ocr_view_edit"]:
                    current_text = st.session_state.merged_pdf_text or ""
                    ...
                    if st.session_state.use_ocr_text:
                        ...
                        st.session_state.merged_pdf_text_native = edited_text
                else:
                    if not st.session_state.merged_pdf_text:
                        st.info(labels["ocr_no_text_yet"])
                    else:
                        st.markdown(st.session_state.merged_pdf_text, unsafe_allow_html=True)

                col_src1, col_src2 = st.columns(2)
                with col_src1:
                    if st.button(labels["ocr_use_native"], key="btn_use_native"):
                        if st.session_state.merged_pdf_text_native:
                            st.session_state.merged_pdf_text = st.session_state.merged_pdf_text_native
                            st.session_state.use_ocr_text = False
                            st.session_state.pdf_summary_result = ""
                            st.session_state.pdf_qa_result = ""
                        else:
                            st.warning("No native extracted text available.")
                with col_src2:
                    if st.button(labels["ocr_use_ocr"], key="btn_use_ocr"):
                        if st.session_state.merged_pdf_text_ocr:
                            st.session_state.merged_pdf_text = st.session_state.merged_pdf_text_ocr
                            st.session_state.use_ocr_text = True
                            st.session_state.pdf_summary_result = ""
                            st.session_state.pdf_qa_result = ""
                        else:
            ...
            # --------- PDF SUMMARY ----------
            st.markdown(f"#### {labels['pdf_summary_section']}")

            merged_bytes = st.session_state.merged_pdf_bytes
            merged_text_for_llm = st.session_state.merged_pdf_text
            ...
            if not merged_bytes:
                ...
                with sum_col1:
                    pdf_summary_prompt = st.text_area(
                        labels["pdf_summary_prompt"],
                        value=st.session_state.pdf_summary_prompt,
                        height=260,
                        key="pdf_summary_prompt",
                    )
                with sum_col2:
                    summary_model_name = st.selectbox(
                        labels["pdf_summary_model"],
                        options=PDF_MODEL_OPTIONS,
                        key="pdf_summary_model",
                    )
                    summary_max_tokens = st.number_input(
                        labels["pdf_summary_max_tokens"],
                        min_value=512,
                        max_value=32000,
                        value=4096,
                        step=512,
                    ...
                    summary_temp = st.slider(
                        labels["pdf_summary_temperature"],
                        min_value=0.0,
                        max_value=1.0,
                        value=0.2,
                        step=0.05,
                        key="pdf_summary_temperature",
                    )

                if st.button(labels["pdf_summary_run"], key="pdf_summary_run_btn"):
                    provider = provider_for_pdf_model(summary_model_name)
                    dummy_agent = AgentConfig(
                        id="pdf-summary",
                        name="PDF Summary",
                        provider=provider,
                        ...
                        temperature=float(summary_temp),
                        user_prompt=pdf_summary_prompt,
                    )
                    try:
                        content = f"""以下為合併後 PDF 的完整文字內容：
...
"""
                        log_event(
                            "info",
                            f"Running PDF summary (provider={provider}, model={summary_model_name})",
                        )
                        res = call_llm(
                            dummy_agent,
                            PDF_SUMMARY_SYSTEM_PROMPT_ZH,
                        ...
                        st.session_state.pdf_summary_result = res.get("text") or ""
                        log_event("success", "PDF summary completed.")
                    except Exception as e:
                        st.error(f"PDF summary failed: {e}")
                        log_event("error", f"PDF summary failed: {e}")

                sum_view_mode = st.radio(
                    labels["pdf_summary_view_mode"],
                    options=[labels["pdf_view_rendered"], labels["pdf_view_raw"]],
                    key="pdf_summary_view_mode",
                )
                summary_result = st.session_state.pdf_summary_result
                if not summary_result:
                    st.info("尚未產生摘要。請先點選「產生摘要與實體表格」。")
                else:
                    if sum_view_mode == labels["pdf_view_rendered"]:
                        st.markdown(summary_result, unsafe_allow_html=True)
                    else:
                        st.code(summary_result, language="markdown")
                    st.download_button(
                        labels["export_md"],
                        data=summary_result,
                        file_name="merged_pdf_summary.md",
                        mime="text/markdown",
                        key="pdf_summary_download",
                    )

            st.markdown("---")

            # --------- PDF Q&A -------------
            st.markdown(f"#### {labels['pdf_qa_section']}")
            ...
            if not merged_bytes:
                ...
                with qa_col1:
                    question = st.text_area(
                        labels["pdf_qa_question"],
                        height=120,
                        key="pdf_qa_question",
                    )
                    pdf_qa_prompt = st.text_area(
                        labels["pdf_qa_prompt"],
                        value=st.session_state.pdf_qa_prompt,
                        height=160,
                        key="pdf_qa_prompt",
                    )
                with qa_col2:
                    ...
                    qa_max_tokens = st.number_input(
                        labels["pdf_qa_max_tokens"],
                        min_value=1024,
                        max_value=32000,
                        value=24000,
                        step=1024,
                        key="pdf_qa_max_tokens",
                    )
                    qa_temp = st.slider(
                        labels["pdf_qa_temperature"],
                        min_value=0.0,
                        max_value=1.0,
                        value=0.2,
                        step=0.05,
                        key="pdf_qa_temperature",
                    )

                if st.button(labels["pdf_qa_run"], key="pdf_qa_run_btn"):
                    if not question.strip():
                        st.warning("請先輸入問題再執行問答。")
                    else:
                        provider = provider_for_pdf_model(qa_model_name)
                        dummy_agent = AgentConfig(
                            id="pdf-qa",
                            name="PDF QA",
                            provider=provider,
                            ...
                            user_prompt=pdf_qa_prompt,
                        )
                        user_content = f"""以下為合併後 PDF 的完整文字內容：
...
"""
                        try:
                            log_event(
                                "info",
                                f"Running PDF Q&A (provider={provider}, model={qa_model_name})",
                            )
                            res = call_llm(
                                dummy_agent,
                                PDF_QA_SYSTEM_PROMPT_ZH,
                            ...
                            st.session_state.pdf_qa_result = res.get("text") or ""
                            log_event("success", "PDF Q&A completed.")
                        except Exception as e:
                            st.error(f"PDF Q&A failed: {e}")
                            log_event("error", f"PDF Q&A failed: {e}")

                qa_view_mode = st.radio(
                    labels["pdf_qa_view_mode"],
                    options=[labels["pdf_view_rendered"], labels["pdf_view_raw"]],
                    key="pdf_qa_view_mode",
                )
                qa_result = st.session_state.pdf_qa_result
                if not qa_result:
                    st.info("尚未有問答結果。請先輸入問題並執行問答。")
                else:
                    if qa_view_mode == labels["pdf_view_rendered"]:
                        st.markdown(qa_result, unsafe_allow_html=True)
                    else:
                        st.code(qa_result, language="markdown")
                    st.download_button(
                        labels["export_md"],
                        data=qa_result,
                        file_name="merged_pdf_qa.md",
                        mime="text/markdown",
                        key="pdf_qa_download",
                    )
